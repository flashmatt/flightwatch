<template>
  <div class="flex flex-col items-center bg-white rounded-xl py-2 px-4">
    ...
    <div class="flex w-full items-center">
      <span class="text-3xl font-extrabold">{{ route[0].iata }}</span>
        ...
        <div class="flex-grow border-t-2 border-gray-400 border-dashed mx-2 relative">
          <svg xmlns="http://www.w3.org/2000/svg" class="w-8 h-8 rotate-90 absolute -top-4" :style="{ left: `${percentage}%` }" viewBox="0 0 32 32">
    ...
    <div class="flex justify-between w-full">
      <span class="font-light text-sm">{{ departureDistance.toFixed(2) }} NM</span>
      <span class="font-light text-sm">{{ arrivalDistance.toFixed(2) }} NM</span>
    </div>
...
const percentage = computed(() => {
  ...
  let percentage = ((initialDistance.value - arrivalDistance.value) / initialDistance.value) * 100;
