...
const useSprites = () => {
  const svgs = ref({});
  const getBaseShape = (name) => {
    if (baseShapes[name]) {
      return baseShapes[name];
    }
    console.log("No base shape found for " + name);
  };
  const generateSvg = (name) => {
    const json = getBaseShape(name);

    if (!json) {
      console.warn(`SVG shape "${name}" not found.`);
      return null;
    }
    ...
    svg.setAttribute("width", json.size ? json.size[0] : json.w);
    svg.setAttribute("height", json.size ? json.size[1] : json.h);

    // Apply optional attributes
    if (json.strokeScale) {
      svg.setAttribute("stroke-width", json.strokeScale);
    }
    if (json.transform) {
      svg.setAttribute("transform", json.transform);
    }
    if (json.noAspect) {
    ...
    if (Array.isArray(json.path)) {
      ...
      const path = document.createElementNS(svgNS, "path");
      path.setAttribute("d", json.path);
      path.setAttribute("fill", "white");
      path.setAttribute("stroke", "white");
      svg.appendChild(path);
    }

    // Handle accent paths if present
    if (json.accent) {
      if (Array.isArray(json.accent)) {
        json.accent.forEach((a) => {
          const path = document.createElementNS(svgNS, "path");
        ...
        const path = document.createElementNS(svgNS, "path");
        path.setAttribute("d", json.accent);
        path.setAttribute("fill", "white");
        path.setAttribute("stroke", "white");
        svg.appendChild(path);
      }
    ...
    // Optional: Remove line breaks and unnecessary whitespace for optimization
    svgString = svgString.replace(/\n+/g, "").replace(/>\s+</g, "><").trim();

    // Encode the SVG string
    const encodedSvg = encodeURIComponent(svgString)
      // Replace certain characters with their escape codes
      .replace(/'/g, "%27")
      .replace(/"/g, "%22");
  ...
  const addSvg = (name) => {
    svgs.value[name] = generateSvg(name);
  };
  ...
  const getTypeDesignator = (type) => {
    if (type === null || type === undefined) {
      type = "unknown";
    }
    if (typeDesignators[type]) {
      return typeDesignators[type][0];
    }
  };

  const getSvgFromAircraft = (type) => {
    const designator = getTypeDesignator(type);
    if (designator) {
      return getSvg(designator);
    }
    return getSvg("unknown");
  };
  ...
  const baseShapes = {
    ...
    blimp: {
      ...
      accent:
        "m12.48-.52-.18.25c-4.72 7.7-.1 26.23-.1 26.23l-.18.13m.03-26.57.25.21c4.72 7.7.1 26.23.1 26.23l.18.12",
      accentMult: 0.8,
    },
    balloon: {
      id: 2,
    ...
    f18: {
      ...
      accent:
        "M15.04 23.81l-1.42-.66s-.22-3.7.2-4.85m-4.84 5.5l1.4-.66s.23-3.69-.19-4.85",
    },
    e3awacs: {
      id: 9,
      w: 36,
    ...
    l159: {
      ...
      accent:
        "M10.81 13.65l.75-.07c.45.92.7 2.13.7 2.13l-.01 10.4v-10.4s.24-1.21.68-2.13l.75.07",
    },
    mirage: {
      ...
      accent:
        "M 9.90,9.57 C 10.50,12.57 10.80,15.90 10.80,15.90 l 0.20,4.50 0.01,3.21 M 14.59,9.57 C 13.99,12.57 13.69,15.90 13.69,15.90 l -0.20,4.50 -0.01,3.21",
    },
    sb39: {
      // Saab JAS-39 Gripen (ICAO SB39)
      viewBox: "-4.1 0 32 32",
      id: 25,
      w: 29,
      h: 29,
      ...
      accent:
        "M13.52 24.7s-.16-.4-.14-.5c.01-.08.24-1 .28-2.33 0 0 0-1.33-.08-2.5-.07-1.18 0-2.77.14-4.16.14-1.4.12-4.17-.05-5.26-.16-1.09-.2-1.64-.2-1.64m-3.1 16.4s.16-.41.14-.5c-.02-.1-.24-1.02-.28-2.34 0 0 0-1.33.08-2.5.07-1.18 0-2.77-.14-4.16a29.67 29.67 0 01.24-6.9",
    },
    ...
    tornado: {
      ...
      accent:
        "M15.47 10.17l-.72.86s-.35 1.3-.4 2.43c0 0-.1.88-.04 2.09l-.06 4s.01 1.78-.4 4.2l-.52 1.24M8.71 10.17l.72.86s.34 1.3.39 2.43c0 0 .1.88.05 2.09l.06 4s-.02 1.78.39 4.2l.52 1.24",
    },
    ...
    typhoon: {
      ...
      accent:
        "M 13.69,6.55 13.86,12.79 l 0.32,5.97 c 0,0 0.12,4.31 -0.01,6.05 m -1.99,-1.64 v 7.07 M 10.21,24.81 C 10.02,23.09 10.18,18.68 10.18,18.68 l 0.34,-5.94 0.18,-6.15",
    },
    rafale: {
      id: 30,
      w: 25,
    ...
    hunter: {
      ...
      accent:
        "M14.36 6.6l-1.16.77s-.37.02-.44 2.33c0 0 .06 2.16.3 3.47 0 0 .14 1.68.1 3.54l1.32 1.36M9.3 6.6l1.16.78s.37.02.43 2.33c0 0-.05 2.16-.3 3.47 0 0-.13 1.68-.09 3.54l-1.32 1.36",
    },
    ...
    verhees: {
      ...
      accent:
        "M-.63 19.22h9.1l3.84 2.68 3.85-2.68h8.82M15.35 5.44l-1.07 1.22m-5-1.22l1.06 1.22.79 8.33 1.18 6.9.36 1.35m-.72 0l.36-1.34 1.19-6.91.78-8.33h-3.94",
    },
    ...
    m326: {
      ...
      accent:
        "M14.45 6.98l-.88 1.83v.57s-1.06 4.84.7 8.03m-2.07 1.55V26M9.95 7.28l.89 1.83v.57s1.05 4.84-.7 8.03",
    },
    ...
    miragef1: {
      ...
      accent:
        "M13.38 8.55l.37 1.15s.37 1.64.21 3.21c-.15 1.58-.56 6.47-.36 7.19 0 0 .2 2.83-.52 6.96l-.1 1.2M11.01 8.55l-.36 1.15s-.37 1.64-.21 3.21c.15 1.58.55 6.47.35 7.19 0 0-.18 2.83.53 6.96l.1 1.2",
    },
    unknown: {
      id: 64,
      w: 22,
  ...
  const typeDesignators = {
    ...
    AT76: ["twin_large", 0.78],
    C77R: ["cessna", 0.78],

    A318: ["airliner", 0.9], // shortened a320 68t
    ...
    B772: ["heavy_2e", 1.0], // all pretty similar except for length
    B773: ["heavy_2e", 1.02],
    B77L: ["heavy_2e", 1.02],
    B77W: ["heavy_2e", 1.04],
    ...
    B721: ["jet_swept", 1.1], // 80t
    B722: ["jet_swept", 1.1], // 80t
